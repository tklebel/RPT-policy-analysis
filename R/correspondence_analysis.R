...
plot_ca <- function(object, font_size = 3, dimensions = c(1, 2),
  ...
  augmented_data$col_data <- augmented_data$col_data %>%
    mutate(sup_var = case_when(
      str_detect(sup_var, "Supple") ~ "Supplementary Variables",
      str_detect(rowname, "\\+\\+") ~ "Indicator present",
      str_detect(rowname, "--") ~ "Indicator not present"
    ))
